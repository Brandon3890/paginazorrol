// lib/transbank-service.ts - VERSIÓN CORREGIDA FINAL
...
class TransbankService {
  ...
  constructor() {
    ...
    // 🔥 SOLO usar TRANSBANK_ENVIRONMENT (NO NODE_ENV)
    ...
    if (!this.commerceCode || !this.apiKey) {
      console.error('❌ Faltan credenciales de Transbank en .env')
    }
  }
  ...
  generateBuyOrder(): string {
    ...
    return `TBK${timestamp}${random}`.slice(0, 26)
  }

  generateSessionId(): string {
    return `SES${Date.now()}${Math.random()
      .toString(36)
      .substring(2, 15)}`.slice(0, 61)
  }
