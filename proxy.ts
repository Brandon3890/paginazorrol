...
// Verificar si una ruta requiere auth
...
export async function proxy(request: NextRequest) {
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            'Demasiados intentos de login. Intenta nuevamente en 30 minutos.',
  ...
  if (needsAuth || isAdmin) {
    ...
    try {
      ...
      // Rate limit admin
      if (isAdmin) {
        const adminLimitResult = adminRateLimiter.attempt(
          ...
          return NextResponse.json(
            {
              success: false,
              message:
                'Demasiadas acciones administrativas. Espera un momento.'
            },
            { status: 429 }
          )
        }
